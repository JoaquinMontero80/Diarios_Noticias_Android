...
public class LectorRssEconomia extends AsyncTask<Void, Void, Void>
{
    ...
    /*
        el pais : http://ep00.epimg.net/rss/tags/ultimas_noticias.xml    lo ultimo
        el mundo : https://e00-elmundo.uecdn.es/elmundo/rss/portada.xml  portada
        abc : https://www.abc.es/rss/feeds/abc_ultima.xml    ultima hora
        20 minutos : https://www.20minutos.es/rss/
     */
    // Direcciones de los feed noticias generales
    String direccion  = ConstantesDiarios.RSS_EXPANSION;
    // Trabajar con la red
    URL url;
    ProgressDialog progressDialog;

    // Al constructor le añado el recyclerView como segundo parametro
    public LectorRssEconomia(Context context, RecyclerView recyclerview) {
            ...
            for(int i=0; i<items.getLength(); i++)
            {
             ...
             /*           else if(actual.getNodeName().equalsIgnoreCase("content:encoded"))
                        ...
                        else if(actual.getNodeName().equalsIgnoreCase("media:content"))
                        {
                            ...
                            String miUrl = actual.getAttributes().item(0).getTextContent();
