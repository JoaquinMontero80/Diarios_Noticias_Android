...
public class LectorRssConfidencial extends AsyncTask<Void, Void, Void>
{
    ...
    String direccion  = ConstantesDiarios.RSS_ELCONFIDENCIAL;
    // Trabajar con la red
    URL url;
    ProgressDialog progressDialog;

    // Al constructor le añado el recyclerView como segundo parametro
    public LectorRssConfidencial(Context context, RecyclerView recyclerview) {
            ...
            for(int i=0; i<items.getLength(); i++)
            {
                ...
                if(hijoActual.getNodeName().equalsIgnoreCase("entry"))
                {
                    ...
                    for(int j=0; j<itemsChild.getLength(); j++)
                    {
                        ...
                        {
                            String miEnlace = actual.getAttributes().item(2).getTextContent();
                            noticia.setEnlace(miEnlace);
             ...
             /*           else if(actual.getNodeName().equalsIgnoreCase("content:encoded"))
                        ...
                        else if(actual.getNodeName().equalsIgnoreCase("mi:dateTimeWritten"))
                        {
